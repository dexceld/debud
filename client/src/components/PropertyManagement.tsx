                          ...
                          {!t_tnt ? (
                            ...
                            <>
                              ...
                              <div style={{ background:'linear-gradient(135deg,#EEF2FF,#F5F3FF)', borderRadius:12, padding:'14px 16px', marginBottom:14 }}>
                                ...
                                <div style={{ borderTop:'1px solid #C7D2FE', paddingTop:10, display:'flex', justifyContent:'flex-end' }}>
                                  {(() => {
                                    const hasRenewal = tasks.some(tk => tk.tenancyId===t.id && tk.type==='renewal' && !tk.done)
                                    return (
                                      <button type="button" onClick={() => {
                                        if (!hasRenewal) setTasks(prev => [...prev, {
                                          id:`manual_renewal_${t.id}`, propertyId:t.propertyId, tenantId:t.tenantId, tenancyId:t.id,
                                          type:'renewal', title:'חידוש חוזה', dueDate:t.contractEnd, done:false, doneAt:'', autoGenerated:false
                                        }])
                                        t_tnt.phone ? sendWA(t_tnt.phone, `שלום ${t_tnt.name}, תזכורת לחידוש חוזה השכירות עבור ${p.name}. החוזה מסתיים ב-${t.contractEnd}. האם ברצונך לחדש? 🙏`) : alert('אין טלפון רשום')
                                      }}
                                        style={{ background:'#F0FDF4', border:'1px solid #BBF7D0', color:'#15803D', borderRadius:8, padding:'7px 12px', fontWeight:700, cursor:'pointer', fontSize:13, whiteSpace:'nowrap', display:'flex', alignItems:'center', gap:6 }}>
                                        {hasRenewal ? '✅' : '🔄'} {hasRenewal ? 'שלח תזכורת' : 'חידוש + משימה'}
                                      </button>
                                    )
                                  })()}
                                </div>
