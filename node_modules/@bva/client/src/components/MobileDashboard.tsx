                ...
                <div style={{display: 'flex', flexDirection: 'column', padding: '0 16px', background: 'white', borderRadius: 12, border: '1px solid #E5E7EB', margin: '0 4px'}}>
                  {allItems.map((item, idx) => {
                    if (item.type === 'entry') {
                      ...
                      const notePreview = trimmedNote.length > 80 ? `${trimmedNote.slice(0, 77)}…` : trimmedNote
                      return (
                        <div key={entry.id}
                          ...
                          style={{ padding: '8px 0', borderBottom: '1px solid #E5E7EB', cursor: 'pointer', display:'flex', alignItems:'center', gap:12, direction:'rtl' }}>
                          <div style={{minWidth:110}}>
                            <div style={{fontSize: 18, fontWeight: 800, color: '#111827'}}>
                              {new Date(entry.startDate).toLocaleDateString('he-IL', {day: '2-digit', month: '2-digit', year: '2-digit'})}
                            </div>
                            <div style={{fontSize: 12, color: '#6B7280'}}>{entry.startTime}–{entry.endTime}</div>
                          </div>
                          <div style={{flex:1, minWidth:0, fontSize: 12, color:'#4B5563', display:'flex', alignItems:'center', gap:6}}>
                            ...
                            {notePreview ? (
                              <span style={{whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{notePreview}</span>
                            ) : (
                              <span style={{color:'#D1D5DB'}}>—</span>
                            )}
                          </div>
                          <div style={{minWidth:90, textAlign:'left', direction:'ltr'}}>
                            {amount > 0 && <div style={{fontSize: 14, fontWeight: 700, color: '#059669'}}>₪{amount.toLocaleString(numLocale, {maximumFractionDigits: 0})}</div>}
                            <div style={{fontSize: 13, color: '#7C3AED', fontWeight: 600}}>{hours.toFixed(1)}h</div>
                          </div>
                        </div>
                      )
                    } else {
                      ...
                      const trimmedNote = (charge.notes || '').trim()
                      const notePreview = trimmedNote.length > 80 ? `${trimmedNote.slice(0, 77)}…` : trimmedNote
                      return (
                        <div key={charge.id}
                          ...
                          style={{ padding: '8px 0', borderBottom: '1px solid #E5E7EB', cursor: 'pointer', background: '#faf5ff', display:'flex', alignItems:'center', gap:12, direction:'rtl' }}>
                          <div style={{minWidth:110}}>
                            <div style={{fontSize: 18, fontWeight: 800, color: '#111827'}}>
                              {new Date(charge.date).toLocaleDateString('he-IL', {day:'2-digit', month:'2-digit', year: '2-digit'})}
                            </div>
                            <div style={{display:'flex', alignItems:'center', gap:6, flexWrap:'wrap', fontSize:11}}>
                              <span style={{padding:'1px 6px', borderRadius:4, background:'#ede9fe', color:'#7c3aed', fontWeight:600}}>{tag?.name || charge.tagId}</span>
                              {charge.employeeId && (
                                <span style={{padding:'1px 6px', borderRadius:4, background:'#f3e8ff', color:'#6b21a8', fontWeight:600}}>
                                  {employees.find(e => e.id === charge.employeeId)?.name || 'עובד'}
                                </span>
                              )}
                              <span style={{padding:'1px 6px', borderRadius:4, fontWeight:600, background: status === 'paid' ? '#dcfce7' : status === 'invoiced' ? '#dbeafe' : '#fef3c7', color: status === 'paid' ? '#166534' : status === 'invoiced' ? '#1e40af' : '#92400e'}}>
                                {status === 'paid' ? t('statusPaid') : status === 'invoiced' ? t('statusInvoiced') : t('statusPending')}
                              </span>
                            </div>
                          </div>
                          <div style={{flex:1, minWidth:0, fontSize: 12, color:'#4B5563'}}>
                            {notePreview ? (
                              <span style={{whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{notePreview}</span>
                            ) : (
                              <span style={{color:'#D1D5DB'}}>—</span>
                          ...
                          <div style={{minWidth:90, textAlign:'left', direction:'ltr', fontSize: 15, fontWeight: 700, color: '#7c3aed'}}>
